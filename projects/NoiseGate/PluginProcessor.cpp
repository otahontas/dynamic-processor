...
true) #endif .withOutput ("Output", juce::AudioChannelSet::stereo(), true)
                     #endif
                       )
#endif
{
...
NoiseGateAudioProcessor::NoiseGateAudioProcessor()
  ...
  parameterManager.registerParameterCallback(
      Param::ID::Enabled, [this](float newValue, bool force) {
        enabled = newValue > 0.5f;
        // enableRamp.setTarget(enabled ? 1.f : 0.f, force);
      });

  parameterManager.registerParameterCallback(
      Param::ID::Threshold, [this](float newValue, bool /*force*/) {
        noiseGate.setThreshold(newValue);
      });

  parameterManager.registerParameterCallback(
      Param::ID::Ratio,
      [this](float newValue, bool /*force*/) { noiseGate.setRatio(newValue); });

  parameterManager.registerParameterCallback(
      Param::ID::Attack, [this](float newValue, bool /*force*/) {
        noiseGate.setAttack(newValue);
      });

  parameterManager.registerParameterCallback(
      Param::ID::Release, [this](float newValue, bool /*force*/) {
...
void NoiseGateAudioProcessor::changeProgramName(int index,
...
void NoiseGateAudioProcessor::prepareToPlay(double sampleRate,
  ...
  // double dsp::ProcessSpec::sampleRate = sampleRate;

  noiseGate.prepare({sampleRate, (uint32)samplesPerBlock, (uint32)numChannels});
...
#ifndef JucePlugin_PreferredChannelConfigurations
bool NoiseGateAudioProcessor::isBusesLayoutSupported(
...
void NoiseGateAudioProcessor::processBlock(juce::AudioBuffer<float> &buffer,
  ...
  auto inoutBlock = dsp::AudioBlock<float>(buffer).getSubsetChannelBlock(
      0, (size_t)numChannels);
  ...
  for (int channel = 0; channel < totalNumInputChannels; ++channel) {
    ...
    // buffer.copyFrom(channel, 0, inoutBlock.getChannelPointer(channel),
    // buffer.getNumSamples()); auto block =
    // inoutBlock.getSingleChannelBlock(channel); buffer.copyFrom(channel, 0,
    // inoutBlock.getSingleChannelBlock(channel), buffer.getNumSamples());
    /*for (auto sample = 0; sample < buffer.getNumSamples(); ++sample) {
        channelData[sample] = noiseGate.processSample(channel,
    channelData[sample]);
    }*/
  }
}

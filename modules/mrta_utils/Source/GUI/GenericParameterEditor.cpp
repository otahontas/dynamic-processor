...
GenericParameterEditor::GenericParameterEditor(
    mrta::ParameterManager &parameterManager, int pwh,
    const juce::StringArray &pids)
    : parameterWidgetHeight{pwh}, parameterIDs{pids} {
  juce::AudioProcessorValueTreeState &apvts{parameterManager.getAPVTS()};

  const std::vector<mrta::ParameterInfo> &originalParameters{
  ...
  std::transform(parameters.begin(), parameters.end(),
                 std::back_inserter(parameterComponents),
                 [&apvts](const mrta::ParameterInfo &p) {
                   ...
                   case mrta::ParameterInfo::Choice:
                     ptr =
                         std::make_unique<mrta::ParameterComboBox>(p.ID, apvts);
                     break;
  ...
  std::transform(
      parameters.begin(), parameters.end(), std::back_inserter(parameterLabels),
      [](const mrta::ParameterInfo &p) {
  ...
  std::for_each(parameterComponents.begin(), parameterComponents.end(),
                [this](const auto &pc) { addAndMakeVisible(pc.get()); });
  std::for_each(parameterLabels.begin(), parameterLabels.end(),
                [this](const auto &pl) { addAndMakeVisible(pl.get()); });
}
